...
back_translation, entities, flags. Each entity must have kind, source_text, translated_text."""
...
def parse_review_output(text: str) -> Review:
    ...
    for entity in value["entities"]:
        expected_entity_keys = {"kind", "source_text", "translated_text"}
        ...
        if not isinstance(entity["source_text"], str) or not isinstance(
            entity["translated_text"], str
        ):
            raise ProviderOutputError("entity text fields must be strings")
        entities.append(
            CriticalEntity(
                kind=entity["kind"],
                source_text=entity["source_text"].strip(),
                translated_text=entity["translated_text"].strip(),
            )
        )
