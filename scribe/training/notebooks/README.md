...
installs deps, and calls `scribe/training/scripts/*` steps; all real logic lives in
`scribe/training/gec/`. The notebooks are **generated**: edit
`scribe/training/scripts/build_notebooks.py` and run `python scribe/training/scripts/build_notebooks.py`
to rebuild them (don't hand-edit the `NN_*.ipynb` files).

## Profile

The bootstrap cell sets `PROFILE = 'smoke'`. Change it to `'full'` for the real
ViMedCSS run. The profile (`scribe/training/gec/profiles.py`) fixes every run-size:
...
`python scribe/training/scripts/run_pipeline.py --profile {smoke,full} --stage {all,data,synth,train,eval}`
